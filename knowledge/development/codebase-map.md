...
- Source of truth: `devmap/areas.jsonl` (one area per line, with version-bounded
  `layouts`) and `devmap/versions.json` (eras + representative tags).
- Generated: `registry/codemap-by-version.json` — the resolved map for every
  representative tag (`0.6.6, 0.7.3, 0.8.5, 0.9.2, 0.10.1, latest`). Rebuilt by
...
- When you discover a path that moved or a missing area, update
  `devmap/areas.jsonl` (add a version-bounded layout) and rerun
...
- `knowledge/architecture/vllm-architecture-overview.md`
- `devmap/areas.jsonl`, `devmap/versions.json`

## Source

- `devmap/areas.jsonl` (version-tagged), `scripts/yax.py` (`where` command)
